...
def make_drawing_path_file_path(job_id):
    """
    main.py가 실제 draw_path()에 넘길 순서 그대로 저장하는 UI용 경로 JSON 경로를 만든다.
    """
    ...
    return DRAWING_PROGRESS_FILE_ROOT / f"{safe_job_id}_actual_drawing_path.json"
...
def start_drawing_path_json_monitor(
    path_file,
    job_id,
    progress_callback,
    stop_event,
    ...
    """
    main.py가 생성한 actual drawing path JSON을 감시해서 서버로 업로드한다.
    """
    ...
    def monitor_worker():
        print("=" * 70, flush=True)
        print("[Robot Adapter][ActualPath] monitor 시작", flush=True)
        print(f"[Robot Adapter][ActualPath] file: {path_file}", flush=True)
        print("=" * 70, flush=True)

        while not stop_event.is_set():
            data = read_valid_path_json()
            ...
            try:
                upload_result = prepare_progress_json_for_dashboard(
                    job_id=job_id,
                    source_json_path=path_file,
                    local_json_path=str(path_file),
                    local_json_url="",
                    suffix="actual_drawing_path",
                )

                state["uploaded"] = True
                state["result"] = upload_result

                drawing_path_url = upload_result.get("drawing_path_json_url", "")
                drawing_path_path = upload_result.get("drawing_path_json_path", "")

                report_progress(
                    progress_callback,
                    job_id=job_id,
                    status_text="실제 main.py 드로잉 경로 반영 완료",
                    step="6 / 6",
                    progress=90,
                    extra_data={
                        "drawingPhase": "path_ready",
                        "drawingPathJsonPath": drawing_path_path,
                        "drawingPathJsonUrl": drawing_path_url,
                        "actualDrawingPathFile": str(path_file),
                    },
                )

                print("=" * 70, flush=True)
                print("[Robot Adapter][ActualPath] actual 경로 JSON 업로드 완료", flush=True)
                print(f"[Robot Adapter][ActualPath] url: {drawing_path_url}", flush=True)
                print("=" * 70, flush=True)
                return
                ...
                state["error"] = str(e)
                print("=" * 70, flush=True)
                print("[Robot Adapter][ActualPath][WARN] actual 경로 JSON 업로드 실패", flush=True)
                print(f"[Robot Adapter][ActualPath][WARN] {e}", flush=True)
                print("=" * 70, flush=True)
                time.sleep(1.0)

        print("[Robot Adapter][ActualPath] monitor 종료", flush=True)

    monitor_thread = threading.Thread(
        target=monitor_worker,
        name=f"drawing-path-json-monitor-{sanitize_filename_text(job_id)}",
        daemon=True,
    )

    monitor_thread.start()

    return monitor_thread, state
...
def run_preview_main(
    ...
    # 중요:
    # preview 단계의 경로 JSON은 실제 main.py가 로봇에 넘기는 경로가 아니다.
    # 이전 버전에서는 여기서 preview_robot_path.json을 drawingPathJsonUrl로 올려서
    # 대시보드가 "추정 경로"를 먼저 따라가는 문제가 있었다.
    # 이제 preview 단계에서는 변환 이미지 확인용 previewJsonPath만 기록하고,
    # canvas용 drawingPathJsonUrl은 main.py가 만든 actual 경로가 준비된 뒤에만 올린다.
    report_progress(
        progress_callback,
        job_id=job_id,
        status_text="preview 경로 생성 완료 - 실제 드로잉 경로 대기",
        step="5 / 6",
        progress=80,
        extra_data={
...
def run_robot_main(
    ...
    drawing_path_file = make_drawing_path_file_path(job_id)

    # 이전 작업 progress/path 파일이 남아 있으면 삭제
    for old_file, old_label in [
        (drawing_progress_file, "progress"),
        (drawing_path_file, "actual path"),
    ]:
        if old_file.exists():
            try:
                old_file.unlink()
    ...
    print(f"[Robot Adapter][Robot] path file     : {drawing_path_file}")
    print("[Robot Adapter][Robot] 주의: 실제 로봇이 움직입니다.")
    print("=" * 70)

    progress_stop_event = threading.Event()
    progress_monitor_thread = None
    path_json_stop_event = threading.Event()
    path_json_monitor_thread = None
    path_json_monitor_state = None

    env_keys = [
        "COBOT1_DRAWING_JOB_ID",
        "COBOT1_DRAWING_PROGRESS_FILE",
        "COBOT1_DRAWING_PATH_FILE",
        "COBOT1_INPUT_IMAGE",
        "COBOT1_DRAWING_PATH_READY_WAIT_SEC",
    ]
    ...
    try:
        ...
        print(f"[Robot Adapter][Robot] env COBOT1_DRAWING_PATH_FILE    : {drawing_path_file}")
        print(f"[Robot Adapter][Robot] env COBOT1_INPUT_IMAGE          : {ALGORITHM_INPUT_IMAGE}")
        print("=" * 70)
        ...
        os.environ["COBOT1_DRAWING_PATH_FILE"] = str(drawing_path_file)
        os.environ["COBOT1_INPUT_IMAGE"] = str(ALGORITHM_INPUT_IMAGE)
        os.environ.setdefault("COBOT1_DRAWING_PATH_READY_WAIT_SEC", "3.0")

        path_json_monitor_thread, path_json_monitor_state = start_drawing_path_json_monitor(
            path_file=drawing_path_file,
            job_id=job_id,
            progress_callback=progress_callback,
            stop_event=path_json_stop_event,
        )
        ...
        path_json_stop_event.set()

        if progress_monitor_thread is not None:
            progress_monitor_thread.join(timeout=2.0)

        if path_json_monitor_thread is not None:
            path_json_monitor_thread.join(timeout=2.0)
    ...
    path_upload_result = {}
    if path_json_monitor_state and isinstance(path_json_monitor_state.get("result"), dict):
        path_upload_result = path_json_monitor_state.get("result") or {}

    actual_drawing_path_url = path_upload_result.get("drawing_path_json_url", "")
    actual_drawing_path_path = path_upload_result.get("drawing_path_json_path", "")
    ...
    report_progress(
        ...
        extra_data={
            ...
            "actualDrawingPathFile": str(drawing_path_file),
            "actualDrawingPathUploaded": bool(path_json_monitor_state and path_json_monitor_state.get("uploaded")),
            "drawingPathJsonPath": actual_drawing_path_path,
            "drawingPathJsonUrl": actual_drawing_path_url,
        },
    )
    ...
    print(f"[Robot Adapter][Robot] drawing_path_file    : {drawing_path_file}")
    print("=" * 70)
    ...
    return {
        ...
        "robotExecuted": True,
        "drawingProgressFile": str(drawing_progress_file),
        "drawingPathFile": str(drawing_path_file),
        "drawing_path_json_path": actual_drawing_path_path,
        "drawing_path_json_url": actual_drawing_path_url,
    }
